...
function run_sql {
    psql -h localhost -U tealbase_admin -d postgres "$@"
}
...
function ship_logs {
    ...
    curl -sf -X POST "https://$REPORTING_PROJECT_REF.tealbase.co/rest/v1/error_logs" \
...
swap_postgres_and_tealbase_admin() {
    run_sql <<'EOSQL'
...
EOSQL
    ...
    retry 8 pg_isready -h localhost -U tealbase_admin

    run_sql <<'EOSQL'
set statement_timeout = '600s';
begin;
create role tealbase_tmp superuser;
set session authorization tealbase_tmp;

-- to handle snowflakes that happened in the past
revoke tealbase_admin from authenticator;
...
declare
  ...
  tealbase_admin_rolpassword text := (select rolpassword from pg_authid where rolname = 'tealbase_admin');
  role_settings jsonb[] := (
    ...
    where a.rolname in ('postgres', 'tealbase_admin')
  );
...
begin
  ...
  alter role postgres rename to tealbase_admin_;
  alter role tealbase_admin rename to postgres;
  alter role tealbase_admin_ rename to tealbase_admin;
  ...
  loop
    ...
    execute(format(
      ...
      case
        when rec.roleid = 'postgres'::regrole then 'tealbase_admin'
        when rec.roleid = 'tealbase_admin'::regrole then 'postgres'
        else rec.roleid::regrole
      end,
      case
        when rec.member = 'postgres'::regrole then 'tealbase_admin'
        when rec.member = 'tealbase_admin'::regrole then 'postgres'
      ...
      case
        when rec.grantor = 'postgres'::regrole then 'tealbase_admin'
        when rec.grantor = 'tealbase_admin'::regrole then 'postgres'
  ...
  execute(format('alter role tealbase_admin password %L;', tealbase_admin_rolpassword));

  -- role settings
  foreach obj in array role_settings
  loop
    execute(format('alter role %I %s reset all',
                   case when obj->>'role' = 'postgres' then 'tealbase_admin' else 'postgres' end,
  ...
  reassign owned by postgres to tealbase_admin;
  ...
  loop
    execute(format('drop user mapping for %I server %I', case when obj->>'role' = 'postgres' then 'tealbase_admin' else 'postgres' end, obj->>'server'));
  end loop;
  foreach obj in array user_mappings
  loop
  ...
  loop
    ...
    loop
      if obj->>'role' in ('postgres', 'tealbase_admin') or rec.grantee::regrole in ('postgres', 'tealbase_admin') then
        execute(format('alter default privileges for role %I %s revoke %s on %s from %s'
                     , case when obj->>'role' = 'postgres' then 'tealbase_admin'
                            when obj->>'role' = 'tealbase_admin' then 'postgres'
                            else obj->>'role'
                       end
                     ...
                     , case when rec.grantee = 'postgres'::regrole then 'tealbase_admin'
                            when rec.grantee = 'tealbase_admin'::regrole then 'postgres'
  ...
  loop
    ...
    for rec in
      select grantor, grantee, privilege_type, is_grantable
      from aclexplode((obj->>'acl')::aclitem[])
      where grantee::regrole in ('postgres', 'tealbase_admin')
    loop
      execute(format('revoke %s on schema %s from %I', rec.privilege_type, (obj->>'oid')::regnamespace, case when rec.grantee = 'postgres'::regrole then 'tealbase_admin' else 'postgres' end));
    end loop;
  end loop;
  foreach obj in array schemas
  loop
  ...
  loop
    ...
    for rec in
      ...
      where grantee::regrole in ('postgres', 'tealbase_admin')
    loop
      execute(format('revoke %s on type %s from %I', rec.privilege_type, (obj->>'oid')::regtype, case when rec.grantee = 'postgres'::regrole then 'tealbase_admin' else 'postgres' end));
    end loop;
  end loop;
  foreach obj in array types
  loop
  ...
  loop
    ...
    loop
      execute(format('revoke %s on %s %s(%s) from %I'
          ...
          , (obj->>'oid')::regproc
          , pg_get_function_identity_arguments((obj->>'oid')::regproc)
          , case when rec.grantee = 'postgres'::regrole then 'tealbase_admin' else 'postgres' end
          ));
    end loop;
  end loop;
  foreach obj in array functions
  ...
  loop
    ...
    for rec in
      ...
      where grantee::regrole in ('postgres', 'tealbase_admin')
    loop
      execute(format('revoke %s on table %s from %I', rec.privilege_type, (obj->>'oid')::oid::regclass, case when rec.grantee = 'postgres'::regrole then 'tealbase_admin' else 'postgres' end));
    end loop;
  end loop;
  foreach obj in array relations
  loop
...
set session authorization tealbase_admin;
drop role tealbase_tmp;
commit;
EOSQL
}
