...
def host():
    ...
    images = list(
        ec2.images.filter(
            Filters=[{"Name": "name", "Values": ["tealbase-postgres-ci-ami-test"]}]
        )
    )
    assert len(images) == 1
    image = images[0]
...
""",
    ...
    def is_healthy(host) -> bool:
        ...
        cmd = host.run(f"curl -sf -k --connect-timeout 30 --max-time 60 https://localhost:8085/health -H 'apikey: {tealbase_admin_key}'")
        if cmd.failed is True:
