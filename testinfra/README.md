...
docker buildx build \
  ...
  --target=extensions \
  --tag=tealbase/postgres:extensions \
  --platform=linux/arm64 \
  --load \
  .
mkdir -p /tmp/extensions ansible/files/extensions
docker save tealbase/postgres:extensions | tar xv -C /tmp/extensions
...
# run tests
AWS_PROFILE=tealbase-dev pytest -vv -s testinfra/test_*.py
```
