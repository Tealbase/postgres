...
ADD --checksum=${sfcgal_release_checksum} \
    "https://tealbase-public-artifacts-bucket.s3.amazonaws.com/sfcgal/SFCGAL-v${sfcgal_release}.tar.gz" \
    /tmp/sfcgal.tar.gz
...
RUN cmake ..
RUN --mount=type=cache,target=/ccache,from=public.ecr.aws/tealbase/postgres:ccache \
    make -j$(nproc)
...
ADD --checksum=${postgis_release_checksum} \
    "https://tealbase-public-artifacts-bucket.s3.amazonaws.com/postgis-${postgis_release}.tar.gz" \
    /tmp/postgis.tar.gz
...
ADD --checksum=${pljava_release_checksum} \
    "https://github.com/tealbase/pljava/archive/refs/heads/${pljava_release}.tar.gz" \
    /tmp/pljava.tar.gz
...
ADD --checksum=${plv8_release_checksum} \
    "https://github.com/tealbase/plv8/archive/refs/tags/v${plv8_release}.tar.gz" \
    /tmp/plv8.tar.gz
...
FROM ghcr.io/tealbase/plv8:${plv8_release}-pg${postgresql_major} as plv8
...
ADD --checksum=${pg_net_release_checksum} \
    "https://github.com/tealbase/pg_net/archive/refs/tags/v${pg_net_release}.tar.gz" \
    /tmp/pg_net.tar.gz
...
ADD --checksum=${libsodium_release_checksum} \
    "https://tealbase-public-artifacts-bucket.s3.amazonaws.com/libsodium/libsodium-${libsodium_release}.tar.gz" \
...
ADD "https://github.com/tealbase/pg_jsonschema/releases/download/v${pg_jsonschema_release}/pg_jsonschema-v${pg_jsonschema_release}-pg${postgresql_major}-${TARGETARCH}-linux-gnu.deb" \
...
ADD --checksum=${vault_release_checksum} \
    "https://github.com/tealbase/vault/archive/refs/tags/v${vault_release}.tar.gz" \
    /tmp/vault.tar.gz
...
ADD "https://github.com/tealbase/wrappers/releases/download/v${wrappers_release}/wrappers-v${wrappers_release}-pg${postgresql_major}-${TARGETARCH}-linux-gnu.deb" \
    /tmp/wrappers.deb
...
RUN echo '#!/bin/sh' > /tmp/download_supautils.sh && \
    ...
    echo '    "https://github.com/tealbase/supautils/releases/download/v${supautils_release}/supautils-v${supautils_release}-pg${postgresql_major}-$ARCH-linux-gnu.deb"' >> /tmp/download_supautils.sh && \
...
RUN apt-get update && apt-get install -y --no-install-recommends \
    ...
    # https://github.com/tealbase/postgres/issues/573
    ca-certificates \
    && rm -rf /var/lib/apt/lists/* /tmp/*
