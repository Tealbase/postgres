...
function setup_postgres {
  ...
  # TODO (darora): walg enablement is temporarily performed here until changes from https://github.com/tealbase/postgres/pull/639 get picked up
  ...
  if [ "${DATA_VOLUME_MOUNTPOINT}" ]; then
    ...
    /usr/local/bin/configure-shim.sh /dist/tealbase-admin-api /opt/tealbase-admin-api
    /opt/tealbase-admin-api optimize db --destination-config-file-path /etc/postgresql-custom/generated-optimizations.conf
...
function setup_credentials {
  ...
  export ADMIN_API_KEY=${ADMIN_API_KEY:-$(jq -r '.["tealbase_admin_key"]' /tmp/init.json)}
