...
# Downloaded from https://github.com/docker-library/postgres/raw/master/15/bullseye/docker-entrypoint.sh
# Changes needed to make adminapi able to read the recovery.signal file:
# -44: 	chmod 00700 "$PGDATA" || :
# +44: 	chmod 00750 "$PGDATA" || :
# 
# We're already including the original file in the base postgres Docker image.
...
docker_setup_env() {
	...
	file_env 'POSTGRES_USER' 'tealbase_admin'
