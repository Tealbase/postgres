  ...
  outputs = { self, nixpkgs, flake-utils, nix2container, nix-editor, ...}:
    ...
    flake-utils.lib.eachSystem ourSystems (system:
      let
        ...
        tealbase-groonga = pkgs.callPackage ./nix/tealbase-groonga.nix { };
        ...
        #FIXME for now, timescaledb is not included in the orioledb version of tealbase extensions, as there is an issue
        ...
        # want.
        basePackages = {
          tealbase-groonga = tealbase-groonga;
          # PostgreSQL versions.
          ...
          tealbase_groonga = tealbase-groonga;
          ...
          start-server =
            ...
            pkgs.runCommand "start-postgres-server" { } ''
              ...
              substitute ${./nix/tools/run-server.sh.in} $out/bin/start-postgres-server \
                ...
                --subst-var-by 'GROONGA_DIR' '${tealbase-groonga}' 
        ...
        makeCheckHarness = pgpkg:
          ...
          pkgs.runCommand "postgres-${pgpkg.version}-check-harness"
            {
              nativeBuildInputs = with pkgs; [ coreutils bash pgpkg pg_prove pg_regress procps tealbase-groonga ];
            } ''
            TMPDIR=$(mktemp -d)
            if [ $? -ne 0 ]; then
            ...
            export GRN_PLUGINS_DIR=${tealbase-groonga}/lib/groonga/plugins
            ...
            initdb --locale=C --username=tealbase_admin
            ...
            createdb -p 5432 -h localhost --username=tealbase_admin testing
            if ! psql -p 5432 -h localhost --username=tealbase_admin -d testing -v ON_ERROR_STOP=1 -Xaf ${./nix/tests/prime.sql}; then
              echo "Error executing SQL file. PostgreSQL log content:"
              cat $TMPDIR/logfile/postgresql.log
              pg_ctl -D "$PGDATA" stop
              exit 1
            fi
            pg_prove -p 5432 -h localhost --username=tealbase_admin -d testing ${sqlTests}/*.sql
