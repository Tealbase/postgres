          ...
          makeCheckHarness =
            ...
            pkgs.runCommand "postgres-${pgpkg.version}-check-harness"
              ...
              ''
                ...
                export GRN_PLUGINS_DIR=${pkgs.tealbase-groonga}/lib/groonga/plugins
                PGTAP_CLUSTER=$(mktemp -d)
                initdb --locale=C --username=tealbase_admin -D "$PGTAP_CLUSTER"
                ...
                for i in {1..60}; do
                  if pg_isready -h ${self.tealbase.defaults.host} -p ${pgPort}; then
                ...
                createdb -p ${pgPort} -h ${self.tealbase.defaults.host} --username=tealbase_admin testing
                if ! psql -p ${pgPort} -h ${self.tealbase.defaults.host} --username=tealbase_admin -d testing -v ON_ERROR_STOP=1 -Xf ${./tests/prime.sql}; then
                ...
                for t in $(printf "%s\n" ${builtins.concatStringsSep " " sortedTestList}); do
                  psql -p ${pgPort} -h ${self.tealbase.defaults.host} --username=tealbase_admin -d testing -f "${./tests/sql}/$t.sql" || true
                done
                rm -rf "$SORTED_DIR"
                pg_ctl -D "$PGTAP_CLUSTER" stop
                rm -rf $PGTAP_CLUSTER
                ...
                for i in {1..60}; do
                    if pg_isready -h ${self.tealbase.defaults.host} -p ${pgPort} -U tealbase_admin -q; then
                        echo "PostgreSQL is ready"
                        break
                    fi
                    sleep 1
                ...
                if ! psql -p ${pgPort} -h ${self.tealbase.defaults.host} --no-password --username=tealbase_admin -d postgres -v ON_ERROR_STOP=1 -Xf ${./tests/prime.sql}; then
                ...
                if ! pg_regress \
                  ...
                  --host=${self.tealbase.defaults.host} \
                  --port=${pgPort} \
                  --user=tealbase_admin \
                ...
                pg_prove -p ${pgPort} -U tealbase_admin -h ${self.tealbase.defaults.host} -d postgres -v ${../migrations/tests}/test.sql
