...
if [ ! -f "$LATEST_VERSION_SCRIPTS" ]; then
  aws s3 cp "s3://${ARTIFACTS_BUCKET_NAME}/upgrades/postgres/tealbase-postgres-${LATEST_PG_VERSION}/pg_upgrade_scripts.tar.gz" "$LATEST_VERSION_SCRIPTS"
fi

if [ ! -f "$LATEST_VERSION_BIN" ]; then
  aws s3 cp "s3://${ARTIFACTS_BUCKET_NAME}/upgrades/postgres/tealbase-postgres-${LATEST_PG_VERSION}/24.04.tar.gz" "$LATEST_VERSION_BIN"
fi
...
docker run -t --name pg_upgrade_test --env-file .env \
   ...
   -p 5432:5432 \
   "tealbase/postgres:${INITIAL_PG_VERSION}"
